...
Rectangle {
    ...
    // Phase 2: rice/LOTM theme integration. Values default to the
    // original R1999_1 palette; the rice-lock-screen launcher exports
    // RICE_R1999_* from the active theme manifest when variant=r1999.
    ...
    // Colors — env-driven with R1999 originals as fallback.
    readonly property color fg: _env("RICE_R1999_FG", "#fdfaf2")
    readonly property color gold: _env("RICE_R1999_ACCENT", "#c9a063")
    readonly property color goldDim: _env("RICE_R1999_ACCENT_DIM", "#8c6d45")
    readonly property color orbitClr: _env("RICE_R1999_ORBIT", "#ffffff")

    // Phase 4 polish switches (all opt-in, qylock defaults preserved).
    // - RICE_R1999_FONT_FAMILY: swap the bundled Cinzel-Bold for a system family.
    // - RICE_R1999_POWER_ROW=off: hide REBOOT/SHUTDOWN (rice dashboard already exposes these).
    // - RICE_R1999_OVERLAY=on: composite a subtle accent-tinted fog above the video, under the UI.
    readonly property string _fontFamily: {
        const override = _env("RICE_R1999_FONT_FAMILY", "");
        return override.length > 0 ? override : serifFont.name;
    }
    readonly property bool _powerRowVisible: {
        const v = _env("RICE_R1999_POWER_ROW", "on").toLowerCase();
        return ["off", "0", "false", "no", "hide"].indexOf(v) < 0;
    }
    readonly property bool _overlayEnabled: {
        const v = _env("RICE_R1999_OVERLAY", "off").toLowerCase();
    ...
    // -> RICE_LOCK_VIDEO), then an R1999-specific override, then the
    // bundled bg.mp4. MediaPlayer accepts file:// or bare paths.
    readonly property string _bgSource: {
        const v = _env("RICE_LOCK_VIDEO", _env("RICE_R1999_BG", ""));
        if (v.length === 0)
            return "bg.mp4";
    ...
    Column {
        ...
        Row {
            ...
            Text {
                ...
                font.pixelSize: 12 * s
                font.letterSpacing: 2 * s
                color: rMa.containsMouse ? root.gold : root.fg
            ...
            Text {
                ...
                font.pixelSize: 12 * s
                font.letterSpacing: 2 * s
                color: pMa.containsMouse ? root.gold : root.fg
